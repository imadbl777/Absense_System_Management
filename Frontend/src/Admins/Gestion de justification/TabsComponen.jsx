...
import { JusCard } from "./Components/JusCard";
...
function TabsComponent({
  ...
  const handleImageView = (justification) => {
    onImageClick(`http://localhost:8000${justification.document_path}`);
  };
  ...
  return (
    <div
      className={`w-full ${darkMode ? "bg-gray-900 text-white" : "bg-gray-50"}`}
    >
      <div
        className={`flex p-1 ${
        ...
        {activeTab === "pending" &&
          pendingJustifications.map((justification) => (
            <JusCard
              ...
              handleJustificationAction={handleJustificationAction}
              type={"pending"}
            />
          ))}
                  ...
                  <div className="flex flex-col space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div className="flex items-center space-x-2 text-sm text-gray-500">
                        ...
                        <span>
                          {formatDate(justification.session.session_date)}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2 text-sm text-gray-500">
                        <FaClock className="text-yellow-400" />
                        <span>
                          Soumis le: {formatDate(justification.submitted_at)}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2 text-sm text-gray-500">
                        <FaClock className="text-orange-400" />
                        <span>
                          Traité le: {formatDate(justification.reviewed_at)}
                        </span>
                      </div>
